<?php 
    ...
    if(isset($_GET['action'])){
        ...
        if($action=="edit" && ($pk)!=''){
            $editSql = "SELECT * FROM `classes` WHERE `id`=?";
        ...
        if(isset($_POST['uid'])){
            ...
            if($ok){
                $sql= "UPDATE `classes` SET `class_name`=?,`course`=?,`department`=? WHERE `id`=?";
                $stmt=mysqli_stmt_init($conn);
                if(!mysqli_stmt_prepare($stmt,$sql)){
                    ...
                    mysqli_stmt_bind_param($stmt,"ssss",$class_name,$course,$department,$pk);
                    if(mysqli_stmt_execute($stmt)){
                        $feedback = "Class has been updated successfully!";
                    }
                }
            }  
        }
                    ...
                    <div class="card card-primary">
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-group p-3">
                                ...
                                <input type="text" name="class_name" id="id_class_name" placeholder="class name" class="form-control" required value="<?php echo $name ?>">
                            </div>
                            <div class="form-group p-3">
                                <label for="id_course">Course:</label>
                                <input type="text" name="course" id="id_course" placeholder="course" class="form-control" required value="<?php echo $course ?>">
                            </div>
                            <div class="form-group p-3">
